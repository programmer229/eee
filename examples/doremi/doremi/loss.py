...
def compute_per_domain_loss(
    losses: torch.Tensor, dataset_idxs: torch.Tensor, doremi_context: DoReMiContext, parallel_context: ParallelContext
    ...
    domain_ids_dp = [
        torch.empty_like(dataset_idxs, device="cuda", memory_format=torch.contiguous_format) for _ in range(dp_size)
    ]
    dist.all_gather(domain_ids_dp, dataset_idxs.contiguous(), group=dp_pg)
...
def compute_domain_loss_per_replicas(
    losses: torch.Tensor, dataset_idxs: torch.Tensor, doremi_context: DoReMiContext
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    dataset_idxs = dataset_idxs.view(-1)

    # NOTE: Calculate total loss per domain
    n_domains = doremi_context.num_domains
    domain_losses = torch.zeros(n_domains, device="cuda")

    assert losses.shape[0] == dataset_idxs.shape[0]
    GLOBAL_BATCH_SIZE = dataset_idxs.shape[0]

    for i in range(GLOBAL_BATCH_SIZE):
        # NOTE: sum the excess losses of all tokens in the batch
        # then add it to the domain loss of the corresponding domain
        domain_losses[dataset_idxs[i]] += losses[i].sum(dim=-1)

    # NOTE: Normalize domain weights
    SEQ_LEN = losses.shape[1]
    samples_per_domain = torch.bincount(dataset_idxs, minlength=n_domains)
    normalized_domain_losses = domain_losses / (samples_per_domain * SEQ_LEN)
...
class DomainLossForProxyTraining:
    ...
    def __call__(self, losses: torch.Tensor, ref_losses: torch.Tensor, dataset_idxs: torch.Tensor):
        assert losses.shape == ref_losses.shape, "losses and ref_losses must have the same shape"
        assert (
            dataset_idxs.shape[0] == losses.shape[0]
        ), "the batch size of dataset_idxs must match the batch size of losses"
        ...
        #     excess_losses, dataset_idxs, self.doremi_context, self.parallel_context
        # )
        normalized_domain_losses, samples_per_domain = compute_domain_loss_per_replicas(
            excess_losses, dataset_idxs, self.doremi_context
        )
...
class CrossEntropyWithPerDomainLoss(nn.Module):
    ...
    def forward(
        ...
        _, domain_losses, samples_per_domain = compute_per_domain_loss(
            per_token_loss, dataset_idxs, self.doremi_context, self.parallel_context
        )
...
class DoReMiLossForProxyTraining(nn.Module):
    ...
    def forward(
        ...
        doremi_loss_outputs = self.doremi_loss(loss, ref_losses, dataset_idxs)
